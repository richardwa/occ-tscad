...
export class Shape3 {
  ...
  union(prop: Shape3 | WithRadius<Shape3>) {
    ...
    if (fillProps.radius) {
      this.shape = this.handleSectionEdges(fillProps, fuse);
    } else {
  ...
  cut(prop: Shape3 | WithRadius<Shape3>) {
    ...
    cut.Build(progress);
    if (fillProps.radius) {
      this.shape = this.handleSectionEdges(fillProps, cut);
    } else {
  ...
  intersect(prop: Shape3 | WithRadius<Shape3>) {
    ...
    if (fillProps.radius) {
      this.shape = this.handleSectionEdges(fillProps, intersect);
    } else {
